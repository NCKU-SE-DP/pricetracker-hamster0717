...
from ..exceptions import EvaluationFailure,InvalidAiInputParamException
...
class LLMClientTemplate(LLMClientBase, ABC):
    ...
    def _generate(self, prompt: MessageInterface) -> Optional[str]:
    ...
    def extract_search_keywords(self, prompt: str) -> str:
        ...
        try:
            ...
            capture_exception(e)
            raise InvalidAiInputParamException(f"Failed to extract keywords: {e}")
    ...
    def generate_summary(self, prompt: str) -> Optional[dict[str, str]]:
        ...
        try:
            ...
            capture_exception(e)
            raise InvalidAiInputParamException(f"Failed to generate a summary based on the prompt: {response}")
    ...
    def evaluate_relevance(self, news_title: str, prompt: str = "民生用品的價格變化") -> Optional[str]:
        ...
        try:
            return Relevance.evaluate_relevance_level(response)
        except InvalidAiInputParamException as e:
            capture_exception(e)
            raise InvalidAiInputParamException(f"Error evaluating relevance: {e}")
