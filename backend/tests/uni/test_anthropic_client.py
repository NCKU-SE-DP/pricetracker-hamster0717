...
from src.llm_client.exceptions import InvalidAiInputParamException, InternalServerErrorException,EvaluationFailure
...
class TestAnthropicClient(unittest.TestCase):
    ...
    def setUpClass(self):
        if RUN_REAL_API_TESTS:
            self.client = AnthropicClient(_api_key=os.getenv("NEWS_ANTROPIC_AI_KEY"))
        else:
            self.client = AnthropicClient(_api_key="fake_api_key")
    ...
    def test_generate_summary(self, mock_generate_text):
        mock_generate_text.return_value = '{"summary": "影響描述", "reason": "原因描述"}'
